...
# NOT MY WORK, JUST A LEARNING PROJECT
# Add single player mode to aim and shoot at target
# Add pause menu from escape key
# Fix score bug (health should say 0 when player loses)
...
YELLOW_SPACESHIP = pygame.transform.rotate(pygame.transform.scale(
    YELLOW_SPACESHIP_IMAGE, (SPACESHIP_W, SPACESHIP_H)), 90)

RED_SPACESHIP_IMAGE = pygame.image.load(os.path.join('Assets', 'spaceship_red.png'))
RED_SPACESHIP = pygame.transform.rotate(pygame.transform.scale(
    RED_SPACESHIP_IMAGE, (SPACESHIP_W, SPACESHIP_H)), 270)
...
def yellow_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VELOCITY > - 7:  # Left
        yellow.x -= VELOCITY
    if keys_pressed[pygame.K_d] and yellow.x + VELOCITY + yellow.width < BORDER.x + SPACESHIP_W/2 - 5:  # Right
        yellow.x += VELOCITY
...
def red_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x - VELOCITY > BORDER.x:  # Left
        red.x -= VELOCITY
    if keys_pressed[pygame.K_RIGHT] and red.x + VELOCITY < WIDTH + - SPACESHIP_W/2 - 3:  # Right
        red.x += VELOCITY
